...
async def test_renko_record_entry_and_exit(repo_and_db):
    ...
    assert completed.total_fees == Decimal("0.0000")
    assert completed.net_pnl == Decimal("0.0265")
...
@pytest.mark.asyncio
async def test_renko_record_exit_applies_fees_to_net_pnl(repo_and_db):
    _, db_mgr = repo_and_db
    repo = RenkoTradeRepository(db_mgr)
    trade_id = make_renko_trade_id(200)
    brick = _brick(index=200, close=2500.0)
    entry = _entry_order()
    await repo.record_entry(
        trade_id=trade_id,
        order=entry,
    ...
    await repo.record_exit(
        trade_id=trade_id,
        order=exit_order,
        brick=_brick(index=201, close=2510.0),
        action_kind="exit_long",
        action_reason="long_exit",
        entry_price=2500.0,
        entry_time=datetime.now(timezone.utc).timestamp(),
        quantity=1.0,
        fees=0.15,
    ...
    completed = await repo.get_trade(trade_id)
    assert completed is not None
    assert completed.realized_pnl == Decimal("0.1000")  # (2510-2500)*1*0.01
    assert completed.total_fees == Decimal("0.1500")
    assert completed.net_pnl == Decimal("-0.0500")
